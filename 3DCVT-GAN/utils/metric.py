...
from utils.SSIMloss import SSIMLoss
...
class MSEMetric(RegressionMetric):
    ...
    def _compute_metric(self, logits: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        return compute_mean_error_metrics(logits, target, func=self.sq_func)
...
class SSIMMetric(RegressionMetric):
    ...
    def __init__(
        self, data_range: torch.Tensor, win_size: int = 7, k1: float = 0.01, k2: float = 0.03, spatial_dims: int = 3
    ):
        super().__init__()
